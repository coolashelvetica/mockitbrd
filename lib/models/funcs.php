...
function fetchAllUsers()
{
    ...
    $stmt = $mysqli->prepare("SELECT
        ...
        FROM ".$db_table_prefix."users");
    $stmt->execute();
    $stmt->bind_result($id, $user, $display, $role, $password, $email, $profile_image, $profile_image_small, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn);

    while ($stmt->fetch()){
        $row[] = array('id' => $id, 'user_name' => $user, 'display_name' => $display, 'role' => $role, 'password' => $password, 'email' => $email, 'profile_image' => $profile_image, 'profile_image_small' => $profile_image_small, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn);
    }
    $stmt->close();
    return ($row);
}
//Retrieve complete user information by username, token or ID
function fetchWallPosts($username=NULL,$token=NULL, $id=NULL)
{
    if($username!=NULL) {
        $column = "user_name";
        $data = $username;
    ...
    $stmt = $mysqli->prepare("SELECT
        ...
        uploads,
        profile_user_id
        FROM ".$db_table_prefix."wall_posts
        WHERE
        $column = ?");
        $stmt->bind_param("s", $data);

    $stmt->execute();
    $stmt->bind_result($post_id, $post, $user_id, $ip, $created, $uploads, $profile_user_id);
    while ($stmt->fetch()){
        $row[] = array('post_id' => $post_id, 'post' => $post, 'user_id' => $user_id, 'ip' => $ip, 'created' => $created, 'uploads' => $uploads, 'profile_user_id' => $profile_user_id);
    }
    $stmt->close();
    return ($row);
}
...
//postToWall
function postToWall($uid, $update, $ip, $profile_id)
{
    global $mysqli,$db_table_prefix;
    $stmt = $mysqli->prepare("INSERT INTO ".$db_table_prefix."wall_posts (
                    post,
                    user_id,
                    ip,
                    created,
    ...
    $stmt->bind_param("sisi", $update, $uid, $ip, $profile_id);
    echo $mysqli->error;
    $result = $stmt->execute();
    echo $mysqli->error;
...
function updateProfileImage($id, $image)
{
    global $mysqli,$db_table_prefix;
    $stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
        SET
